    ...
    def run(self):  
        while True:
            ...
            for i in range(self.uavNum):
                    ...
                    UAV_data = struct.pack("<ddddddddddddddddddddd", (i + 1), self.drones[i].batInfo[0]/1000.0 ,self.drones[i].batInfo[1],
...
class Drone():
    ...
    def open(self):
        try:
            ...
            self.conn = mavutil.mavlink_connection(self.addr, baud=115200)
            self._active = True
    ...
    def update(self):
        if self.cmd_vel_update:
            ...
            self.conn.mav.set_position_target_local_ned_send(0,       # time_boot_ms (not used)
                                                    ...
                                                    mavutil.mavlink.MAV_FRAME_LOCAL_NED, # frame
                                                    0b0000111111111000, # type_mask (only speeds enabled)
                                                    self.cmdVel[0], self.cmdVel[1], self.cmdVel[2], # x, y, z positions (not used)
                                                    self.cmdVel[3], self.cmdVel[4], self.cmdVel[5], # x, y, z velocity in m/s
                                                    0, 0, 0, # x, y, z acceleration (not used)
                                                    0, 0)    # yaw,)
...
class RouterApp():  
    def __init__(self):
        ...
        self.droneNum = len(ComList)
        self.drones = []
	#此处，根据config文件，申明了10台无人机与其udp端口号
        print("MAV NUM === %d"%self.droneNum)
        for i in range(self.droneNum):
                #port=str(ComList[i])
                #drone =  Drone("udpin:192.168.144.12:"+port,i+1)
                drone =  Drone(ComList[i],i+1)
       ...
       # self.matlabReceiver = MatlabReceiver(self,"192.168.122.105",25000)
        self.matlabSender = MatlabSender(self,"127.0.0.1",27000)
        self.matlabReceiver = MatlabReceiver(self,"127.0.0.1",27000)
